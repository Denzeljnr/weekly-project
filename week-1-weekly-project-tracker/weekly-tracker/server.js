...
const { generateLinkedInPost } = require('./linkedin');
...
app.post('/api/entries/:id/upload', upload.array('files'), (req, res) => {
  ...
  req.files.forEach((f, i) => {
    const relPath = paths[i] || f.originalname;
    const dest = path.join(entryDir, relPath);
    fs.mkdirSync(path.dirname(dest), { recursive: true });
    fs.renameSync(f.path, dest);
  });

  res.json({ uploaded: req.files.map((f, i) => paths[i] || f.originalname) });
});

app.post('/api/entries/:id/linkedin-post', async (req, res) => {
  try {
    const entries = readEntries();
    const idx = entries.findIndex(e => e.id === req.params.id);
    if (idx === -1) return res.status(404).json({ error: 'not found' });
    ...
    const recent = entries.slice(Math.max(0, idx - 3), idx); // last 3 entries before this one

    const post = await generateLinkedInPost(entry, recent);
    entry.linkedinPost = post;
    writeEntries(entries);
    res.json({ post });
  } catch (err) {
...
app.post('/api/entries/:id/push', async (req, res) => {
  try {
    ...
    for (const { fullPath, relPath } of files) {
      ...
      const githubPath = `${folder}/${relPath.split(path.sep).join('/')}`;
      await pushFile(githubPath, content, `${entry.week}: ${entry.title}`);
      await new Promise(r => setTimeout(r, 150)); // avoid GitHub's burst rate limit
    }
    ...
    console.error('Push failed:', err); // now prints the real error to your terminal
    res.status(500).json({ error: err.message });
  }
});
